...
# Optimize for torch.compile and CUDAGraph
...
class Spatial_Attention(nn.Module):
    """Spatial Window Self-Attention using FlexAttention.
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        q = q.view(B, H_pad // self.H_sp, self.H_sp, W_pad // self.W_sp, self.W_sp, C)
        q = q.permute(0, 1, 3, 2, 4, 5).contiguous()
        q = q.view(-1, self.window_size, self.num_heads, C // self.num_heads)
        q = q.permute(0, 2, 1, 3)  # (B_win, num_heads, window_size, head_dim)
        
        k = k.view(B, H_pad // self.H_sp, self.H_sp, W_pad // self.W_sp, self.W_sp, C)
        k = k.permute(0, 1, 3, 2, 4, 5).contiguous()
        k = k.view(-1, self.window_size, self.num_heads, C // self.num_heads)
        k = k.permute(0, 2, 1, 3)
        
        v = v.view(B, H_pad // self.H_sp, self.H_sp, W_pad // self.W_sp, self.W_sp, C)
        ...
        attn_output = attn_output.view(
            B, H_pad // self.H_sp, W_pad // self.W_sp, self.H_sp, self.W_sp, C
        )
        attn_output = attn_output.permute(0, 1, 3, 2, 4, 5).contiguous()
        attn_output = attn_output.view(B, H_pad, W_pad, C)
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    """Adaptive Spatial Self-Attention using FlexAttention
    Args:
        dim (int): Number of input channels.
        num_heads (int): Number of attention heads. Default: 6
        split_size (tuple(int)): Height and Width of spatial window.
    ...
    def _compute_shift_schedule(self) -> bool:
        """Determine if this block needs shifted windows."""
        # Exact shift schedule from original implementation
    ...
    def forward(self, x, H, W):
        ...
        # Process V through convolution branch for AIM
        conv_out = self.dwconv(v)
        ...
        # Process each half through respective attention branch
        if self.shift_mask:
            # Apply cyclic shifts for cross-window connections
        ...
        # AIM: Adaptive interaction between attention and convolution
        # C-Map (before sigmoid)
        channel_map = (
            self.channel_interaction(conv_out)
            .permute(0, 2, 3, 1)
            .contiguous()
            .view(B, 1, C)
        )
        ...
        # S-I
        conv_x = torch.sigmoid(spatial_map) * conv_out
...
class Adaptive_Channel_Attention(nn.Module):
    ...
    """Adaptive Channel Self-Attention using FlexAttention
    Args:
        dim (int): Number of input channels.
        num_heads (int): Number of attention heads. Default: 6
        qkv_bias (bool): If True, add a learnable bias to query, key, value. Default: True
    ...
    def forward(self, x, H, W):
        ...
        # AIM gates
        attention_reshape = attn_out.transpose(-2, -1).contiguous().view(B, C, H, W)
        ...
        # S-I
        attn_out = attn_out * torch.sigmoid(spatial_map)
        # C-I
    ...
    def forward(self, x, x_size):
        ...
        for blk in self.blocks:
            if self.use_chk:
                ...
                x = blk(x, x_size)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)  # More compile-friendly than einops
        x = self.conv(x)
...
class DAT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if in_chans == 3:
            rgb_mean = torch.tensor([0.4488, 0.4371, 0.4040], dtype=torch.float32)
            self.register_buffer('mean', rgb_mean.view(1, 3, 1, 1), persistent=False)
        else:
            self.register_buffer('mean', torch.zeros(1, 1, 1, 1, dtype=torch.float32), persistent=False)
        ...
        self.before_RG_norm = nn.LayerNorm(embed_dim)
